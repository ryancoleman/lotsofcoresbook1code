...
extern "C" 
#endif
void pzgetrf_ooc( int *m_in, int *n_in, 
...
if ((is_root)  && (idebug >= 0)) {
  printf("pzgetrf_ooc: mm %d nn %d mb %d nb %d isize %d memsize %d npanels %d nby %d\n",
      mm,nn, mb,nb, isize, memsize, npanels, nby );
};
             ...
             */
             ...
             if (has_work) {
               if (iinfo < 0) {
                 *info = iinfo;
                 if (idebug >= 1) {
                   printf("pzgetrf_ooc: pzgetrf return iinfo=%d\n", iinfo);
                 };
                 return;
                 };
               if (iinfo > 0) {
...
extern "C"
#endif
void pzgetrf_ooc_( int *m_in, int *n_in,
                 double *A, int *ia_in, int *ja_in, int *descA, 
                          int *ipiv_,  int *memsize_in, int *info )
{
  pzgetrf_ooc( m_in, n_in,
               A, ia_in, ja_in, descA,
