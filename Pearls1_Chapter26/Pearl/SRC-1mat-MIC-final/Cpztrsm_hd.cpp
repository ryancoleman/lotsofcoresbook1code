...
extern "C"
#endif
void Cpztrsm_hd( char side, char uplo, char transA, char diag,
       int m,  int n, 
       double *alpha,
       double *A_, int ia, int ja, int *descA,
       double *B_ int ib, int jb, int *descB );
{
  /*
   * Simulate pztrsm triangular solve
  ...
  if (is_m_small) {
    ...
    zalpha = one;
    zbeta = zero;
    scalapack_pzgecopy( notrans, &m, &m, 
        zalpha,  A, &ia, &ja, descA,
        zbeta,   Atmp, &iia, &jja, descAtmp );


    if ((myprow == ia_proc) && (mypcol == ja_proc)) {
        scalapack_zgebs2d( icontxt, &lscope, &ltop, m,m, Atmp, lld );
    }
    else {
    ...
    if (is_left) {
       Cpztrsm(  side, uplo, transA, diag,
              m1, n, one, 
              A,ia1,ja1,descA,
              B,ib1,jb1,descB );

       mm = m2; nn = n; kk = m1;
       Cpzgemm_hdd(notrans,notrans, mm,nn,kk,
