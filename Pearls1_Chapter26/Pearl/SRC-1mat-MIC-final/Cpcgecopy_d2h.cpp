...
extern "C"
#endif
void Cpcgecopy_general(int m, int n, 
         ...
         if ((mm >= 1) && (nn >= 1)) {
           scalapack_pcgeadd( notrans, &mm, &nn, 
            ...
            if ((lmm >= 1) && (lnn >= 1)) {
               ...
               if (!is_ok) {
                 printf("Cpcgecopy(%d,%d): m %d n %d lmm %d lnn %d isizeT %d lrT1 %d lcT1 %d lrB1 %d lcB1 %d iia %d jja %d  iib %d jjb %d \n",
        ...
        float *B, int ib, int jb, int *descB  )
{
   ...
   */
   ...
   Cpcgecopy_general(m,n,A,ia,ja,descA,B,ib,jb,descB, is_device_to_host );
} 
