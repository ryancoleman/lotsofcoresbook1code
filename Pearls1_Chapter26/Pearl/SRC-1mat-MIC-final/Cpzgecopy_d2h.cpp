...
extern "C"
#endif
void Cpzgecopy_general(int m, int n, 
...
#define dT(i,j) (((cuDoubleComplex *)T) + IDX2F(i,j,descT[LLD_]))
...
zero[IMAG_PART] = 0.0;
 ...
 ldB = descB[LLD_];
  ...
  if (!is_ok) {
    printf("Cpzgecopy (%d,%d) :ia %d m %d descA[M_] %d  \n",
            myprow,mypcol,     ia,   m,   descA[M_] );
    printf("Cpzgecopy (%d,%d) :ja %d n %d descA[N_] %d \n",
            myprow,mypcol,     ja,   n,   descA[N_] );
    printf("Cpzgecopy (%d,%d) :ib %d jb %d descB[M_] %d descB[N_] %d\n",
         ...
         if ((mm >= 1) && (nn >= 1)) {
           scalapack_pzgeadd( notrans, &mm, &nn, 
            ...
            if ((lmm >= 1) && (lnn >= 1)) {
               ...
               if (!is_ok) {
                 printf("Cpzgecopy(%d,%d): m %d n %d lmm %d lnn %d isizeT %d lrT1 %d lcT1 %d lrB1 %d lcB1 %d iia %d jja %d  iib %d jjb %d \n",
        ...
        double *B, int ib, int jb, int *descB  )
{
   ...
   */
   ...
   Cpzgecopy_general(m,n,A,ia,ja,descA,B,ib,jb,descB, is_device_to_host );
} 
