...
extern "C" 
#endif
void pcgecopy_hd( F_CHAR_T TRANS, int *m_in, int *n_in,
    float *A, int *ia_in, int *ja_in, int *descA,
    float *dB, int *ib_in, int *jb_in, int *descB )
{
/*
...
Btmp = (float*) malloc( MAX(1,(Locp * Locq))*elmSize );
assert( Btmp != 0 );
...
for( jstart=ja; jstart <= ja + n-1; jstart = jend + 1) {
  ...
  for( istart=ia; istart <= ia + m-1; istart = iend + 1) {
     ...
     pcgeadd_( TRAN,  &mm, &nn,   z_one, A, &iia, &jja, descA,  
