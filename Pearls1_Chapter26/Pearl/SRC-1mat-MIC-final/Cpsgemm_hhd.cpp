...
/*
 * simulate psGEMM but A, B are distributed matrices on host
 ...
 PROFSTART("Cpsgemm_hhd");
  ...
  if (is_k_small) {
    ...
    PROFEND("gemm:copy");
     ...
     if (idebug >= 2) {
       printf("Cpsgemm_hhd: mm %d nn %d kk %d \n", mm,nn,kk);
 ...
 */
     if (idebug >= 1) {
          float *pAtmp = (float *) Atmp;
          float *pBtmp = (float *) Btmp;
          float aij,bij;
          int lld;
    ...
    Cpsgemm_hhd( transA, transB, m,n,k1,
        alpha,   A, ia,ja,descA,
                 B, ib,jb,descB,
        beta,    C, ic,jc,descC );
    ...
    new_beta = one;
    Cpsgemm_hhd( transA, transB, m,n,k2,
...
extern "C"
#endif
void psgemm_hhd( char *transA, char *transB,
          int *m, int *n, int *k,
          float *alpha, float *A, int *ia, int *ja, int *descA,
                         float *B, int *ib, int *jb, int *descB,
          float *beta,  float *C, int *ic, int *jc, int *descC )
