...
void pdgetrf_gpu2(int *m_in, int *n_in, 
...
if ((idebug >= 1) && (is_root)) {
  printf("pdgetrf_gpu2: m %d n %d ia %d ja %d \n",
      m,n,   ia,ja );
};
...
for( jstart=1; jstart <= minmn; jstart = jend + 1) {
  ...
  PROFSTART("d2h:U part");
   ...
   Cpdgecopy_d2h_async( mm,nn,A,iia,jja,descA,
         hA, iah, jah, desc_Ah );
  PROFEND("d2h:U part");
  }
  ...
  PROFSTART("gpu:pdgetrf");
  scalapack_pdgetrf( &mm_lu, &nn_lu, 
   ...
   */
   ...
   PROFSTART("pdgetrf_gpu2:d2h for swap");
   {
     ...
     for (i=1; i <= jsize; i++) {
      ...
      if (is_outside_block) {
        ...
        jah = (jah0-1) + (j+jb);
        if (!has_copied[irow]) {
          Cpdgecopy_d2h( 1, nn, A, iia,jja,descA, 
               hA, iah, jah, desc_Ah );
   ...
   PROFEND("pdgetrf_gpu2:d2h for swap");
   ...
   // cublas_sync_stream();
   PROFSTART("pdgetrf_gpu2: swap on CPU");
   {
     int inc1 = desc_Ah[M_]; 
     int inc2 = desc_Ah[M_];
   ...
   PROFEND("pdgetrf_gpu2: swap on CPU");
   ...
   PROFSTART("pdgetrf_gpu2:h2d for swap");
   {
     int irowmax =  (iah0-1) + jend;
     for (i=1; i <= jsize; i++) {
      int ip = lpvt(i); 
      ...
      if (is_outside_block) {
       ...
       jah = (jah0-1) + (j+jb);
       if (!has_copied[irow]) {
         Cpdgecopy_h2d_async( 1, nn,  hA,iah,jah, desc_Ah, A, iia,jja,descA);
         has_copied[irow] = TRUE;
         };
       };
     };
   ...
   PROFEND("pdgetrf_gpu2:h2d for swap");
         ...
         */
          ...
          if (use_split_gemm) {
            ...
            if ((mm1 >= 1) && (nn1 >= 1) && (kk1 >= 1)) {
              Cpdgemm_hhd( transA,transB,mm1,nn1,kk1,
            ...
            if ((mm2 >= 1) && (nn2 >= 1) && (kk2 >= 1)) {
              Cpdgemm_hhd( transA,transB,mm2,nn2,kk2,
                alpha, hA,iah,jah,desc_Ah,
                       hA,ii2,jj2,desc_Ah,
...
extern "C"
#endif
void pdgetrf_gpu2_( int *m_in, int *n_in,
           double *A, int *ia_in, int *ja_in, int *descA,
           double *Ah, int *iah_in, int *jah_in, int *desc_Ah,
           int *ipiv_,  int *info )
{
    pdgetrf_gpu2( m_in, n_in,
