...
void Cpclaprnt( int m, int n, float *A, int ia, int ja, int *descA, char *cmatnm )
{
  float zalpha[] =  {0.0, 0.0};
  ...
  if (is_root) {
    printf("Cpclaprnt: m %d n %d ia %d ja %d cmatnm %s\n",
                       m,   n,   ia,   ja,   cmatnm );
  };
  ...
  for(i=1; i <= m; i++) {
    for(j=1; j <= n; j++) {
      ...
      scalapack_pcelget( &scope, &top, zalpha, A, &iia, &jja, descA );
      if (is_root) {
