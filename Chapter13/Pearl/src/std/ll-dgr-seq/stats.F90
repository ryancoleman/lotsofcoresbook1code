...
!! and write them to file "stats.out" for the dual grid D2Q9 Lee-Lin multiphase
!! LBM.
!!
...
!  Common Variables
 USE NTypes, ONLY : DBL
 USE Domain, ONLY : iStep, invInitVol, invPi, tCall, xmax_f, xmin_f, ymax_f, ymin_f
 USE FluidParams
 IMPLICIT NONE
...
!  Local Variables
 ...
 INTEGER :: IO_ERR
 REAL(KIND = DBL) :: Pin, Pout, Pdif, Perr, R, Ro, Ref, Ux, Uy, Vef, Vol
...
! Loop through all nodes inside the bubble
 DO j = ymin_f, ymax_f
   DO i = xmin_f, xmax_f
     ...
     IF ( R < (Ro - 2.D0*IntWidth) ) THEN
       Pin = Pin + p_f(i,j)
       nodesIn = nodesIn + 1
     ELSE IF ( R > (Ro + 2.D0*IntWidth) ) THEN
       Pout = Pout + p_f(i,j)
       nodesOut = nodesOut + 1
     END IF
