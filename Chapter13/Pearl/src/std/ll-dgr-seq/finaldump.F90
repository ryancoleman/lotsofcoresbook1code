...
!> @details
!! Generates the final data file for the simulation in the dual grid D2Q9
!! Lee-Lin multiphase LBM, which contains:
!!
!! - Input parameters
...
!  Local Variables
 ...
 REAL(KIND = DBL) :: distroMem, auxMem, totalMem, memUnitG, memUnitF, Mb
...
! Pressure inside and outside the bubble, maximum velocity and effective radius
 DO j = ymin_f, ymax_f
   DO i = xmin_f, xmax_f
     ...
     IF ( rho_f(i,j) < rhoStar ) Vol = Vol + 1.D0
...
! Estimate memory usage (Mb)
 memUnitG  = NX*NY/Mb
 memUnitF  = NX_f*NY_f/Mb

 distroMem = 8.D0*18.D0*( memUnitG + memUnitF )

 auxMem    = 8.D0*( 8.D0*memUnitG + 12.D0*memUnitF ) &
