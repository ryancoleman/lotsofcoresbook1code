...
!> @details
!! Generates the final data file for the simulation in the parallel D2Q9 Lee-Lin
!! multiphase LBM, which contains:
!!
!! - Input parameters
...
!  Local Variables
 ...
 REAL(KIND = DBL) :: Mb, Pdif, Perr, Pin, Pout, Ro, R, Ref, Umax, Vef, Vol
 REAL(KIND = DBL) :: distroMem, auxMem, mpiMem, totalMem
 REAL(KIND = DBL) :: memUnitA, memUnitB, memUnitC, memUnitD
...
! Calculate pressure inside and outside the bubble, maximum velocity in the domain and effective radius
 DO j = yl, yu
   DO i = xl, xu
     ...
     IF ( rho(i,j) < rhoStar ) dataLocal(1) = dataLocal(1) + 1.D0
...
! Estimate memory usage (Mb)
 ...
 memLocal(1) = 8.D0*36.D0*memUnitB
 memLocal(2) = 8.D0*( 9.D0*memUnitB + memUnitC + memUnitD ) + 4.D0*4.D0*memUnitC
 memLocal(3) = 8.D0*( 12.D0*( xsize3 + ysize3 ) )
...
! Save data to file from the master node only
 IF( vproc == master ) THEN
   ...
   IF ( IO_ERR == 0 ) THEN
     ...
     WRITE(10,'(A,ES15.5)')'Interfacial Tension   = ',sigma
