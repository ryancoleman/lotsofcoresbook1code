...
!! the D2Q5/D2Q9 Zheng-Shu-Chew Multiphase LBM.
...
! Common Variables
 USE NTypes, ONLY : DBL
 USE Domain, ONLY : inv12, inv6, ni, now, nxt, xmax, xmin, ymax, ymin
 USE FluidParams
 USE LBMParams
 IMPLICIT NONE
...
! Order parameter
 DO j = ymin, ymax
   DO i = xmin, xmax
     phi(i,j) = f(i,j,0,now) + f(i,j,1,now) + f(i,j,2,now) + f(i,j,3,now) &
              + f(i,j,4,now)
   END DO
 END DO

!--------- Hydrodynamic fields and collision step in the same loop -------------
 DO j = ymin, ymax
   DO i = xmin, xmax
...
! Chemical potential
     muPhi = alpha4*phin*( phin2 - phistar2 ) - kappa_6*lapPhi
...
! Pressure at each node
     p(i,j) = alpha*( phin2*( 3.D0*phin2 - 2.D0*phiStar2 ) - phiStar4 ) &
            - kappa*( phin*lapPhi*inv6 + 0.5D0*gradPhiSq ) + Cs_sq*rhon
...
!--------- Collision and Stream for distribution function g --------------------
