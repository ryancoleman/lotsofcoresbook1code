...
!! and write them to file "stats.out" for the D2Q5/D2Q9 Zheng-Shu-Chew
!! multiphase LBM.
!!
...
!  Common Variables
 USE NTypes,      ONLY : DBL
 USE Domain,      ONLY : invInitVol, invPi, iStep, tCall, xmax, xmin, ymax, ymin
 USE FluidParams, ONLY : bubbles, Convergence, eps, IntWidth, p, phi, sigma, u
 IMPLICIT NONE

!  Local Variables
 INTEGER :: i, j, nodesIn, nodesOut
 INTEGER :: IO_ERR
 REAL(KIND = DBL) :: Pdif, Perr, Pin, Pout, Ro, R, Ref, Ux, Uy, Vef, Vol
...
! Volume and average velocity
     IF ( phi(i,j) >= 0.D0 ) THEN
       Ux  = Ux + u(i,j,1)
       Uy  = Uy + u(i,j,2)
       Vol = Vol + 1.D0
     END IF
...
! Phase conservation and average velocity of discrete phase
 Ux   = Ux/Vol
 Uy   = Uy/Vol
 Ref  = DSQRT( Vol*invPi )
 Vef  = Vol*invInitVol
