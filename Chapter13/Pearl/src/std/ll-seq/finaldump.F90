...
!> @details
!! Generates the final data file for the simulation in the D2Q9 Lee-Lin
...
!  Common Variables
 USE NTypes, ONLY : DBL
 USE Domain, ONLY : invInitVol, invPi, iStep, MaxStep, NX, NY, RelaxStep, xmax, xmin, ymax, ymin
 USE FluidParams
 IMPLICIT NONE
...
! Pressure inside and outside the bubble, maximum velocity and effective radius
 DO j = ymin, ymax
   DO i = xmin, xmax
     ...
     IF ( rho(i,j) < rhoStar ) Vol = Vol + 1.D0
   END DO
 END DO
...
! Estimate memory usage (Mb)
 memUnit  = NX*NY/Mb
 distroMem = 8.D0*36.D0*memUnit
 auxMem    = 8.D0*11.D0*memUnit + 4.D0*4.D0*memUnit
 ...
 IF ( IO_ERR == 0 ) THEN
   ...
   WRITE(10,'(A,I9)')'Length in Y Direction = ',ymin
