...
Obviously we maintained the 2nd derivative capabilities, we also kept
the structure that allows 3 and 5 term functionals.
...
#include <stdlib.h>
#include <stdio.h>
#include "nwxcP_xc_b97.h"

// PW LDA correlation.
void nwxcp_c_pwlda_(double* , double* , double* );

void nwxcP_c_pwlda(double  ra, double rb , double FCLDA[_FCLDA_ELEMENTS]){
  nwxcp_c_pwlda_(&ra, &rb, FCLDA);
};


static void nwxcP_xc_gss3tar(double r, double g, double alpha, double *a, 
...
static void nwxcP_xc_gss5tar(double r, double g, double alpha, double *a, 
                             double *f, double *fr, double *fg, 
                             double *frr, double *frg, double *fgg)
{
  // B97 like 
...
static void nwxcP_xc_gab3tar(double ra, double rb, double ga, double gb,
                             double alpha, double *a, 
                             double  *f, double *fra, double *frb,
                             double *fga, double *fgb,
                             double *frara, double *frarb, double *frbrb,
                             double *fraga, double *fragb,
                             double *frbga, double *frbgb,
...
static void nwxcP_xc_gab5tar(double ra, double rb, double ga, double gb,
                             double alpha, double *a, 
                             double *f, double *fra, double *frb,
                             double *fga, double *fgb,
                             double *frara, double *frarb, double *frbrb,
...
void nwxcp_x_b97_(double *rho_a,double *rho_b, double *ScalGGAXin,
                  double *tol_rho, double *FX, double *sol)
{
  double ra   = rho_a[0];
  double rb   = rho_b[0];
  double ScalGGAX = *ScalGGAXin;
  double gaa  =  rho_a[1]; // gamma_aa = G_AA
  double gbb  =  rho_b[1]; // gamma_bb = G_BB

  int    max_pow_u = (int)sol[0];
  ...
  if(ra > *tol_rho ) {
    if(max_pow_u == 2 ){
      nwxcP_xc_gss3tar(ra,gaa,gamma_xss,xss,
                      &exaa,&Gex_aa_ra,&Gex_aa_gaa,
                      &Gex_aa_rara,&Gex_aa_ragaa,&Gex_aa_gaagaa);
    }else{
      nwxcP_xc_gss5tar(ra,gaa,gamma_xss,xss,
                      &exaa,&Gex_aa_ra,&Gex_aa_gaa,
                      &Gex_aa_rara,&Gex_aa_ragaa,&Gex_aa_gaagaa);
    }
  ...
  if(rb > *tol_rho ) {
    if(max_pow_u == 2 ){
      nwxcP_xc_gss3tar(rb,gbb,gamma_xss,xss,
                      &exbb,&Gex_bb_rb,&Gex_bb_gbb,
                      &Gex_bb_rbrb,&Gex_bb_rbgbb,&Gex_bb_gbbgbb);
    }else{
      nwxcP_xc_gss5tar(rb,gbb,gamma_xss,xss,
                      &exbb,&Gex_bb_rb,&Gex_bb_gbb,
                      &Gex_bb_rbrb,&Gex_bb_rbgbb,&Gex_bb_gbbgbb);
    }
...
void nwxcp_c_b97_(double *rho_a,double *rho_b, double *ScalGGACin, 
...
{ 
  ...
  if(ra > *tol_rho) {
    if(max_pow_u == 2 ){
      nwxcP_xc_gss3tar(ra,gaa,gamma_css,css,&gcaa,
                       &Ggc_aa_ra,&Ggc_aa_gaa,
                       &Ggc_aa_rara,&Ggc_aa_ragaa,&Ggc_aa_gaagaa);
    }else{
      nwxcP_xc_gss5tar(ra,gaa,gamma_css,css,&gcaa,
                       &Ggc_aa_ra,&Ggc_aa_gaa,
                       &Ggc_aa_rara,&Ggc_aa_ragaa,&Ggc_aa_gaagaa);
    }
  } else {
    gcaa = css[0] + css[1] + css[2]; // u -> 1 if s -> infinity
    gcaa = 0.0; // Elda(r)*gcaa -> 0 if r -> 0 hence OK to zero gcaa
    Ggc_aa_ra = 0.0; 
    Ggc_aa_gaa = 0.0; 
    Ggc_aa_rara = 0.0;
    Ggc_aa_ragaa = 0.0;
  ...
  if(rb > *tol_rho) {
    if(max_pow_u == 2 ){
      nwxcP_xc_gss3tar(rb,gbb,gamma_css,css,&gcbb,
                       &Ggc_bb_rb,&Ggc_bb_gbb,
                       &Ggc_bb_rbrb,&Ggc_bb_rbgbb,&Ggc_bb_gbbgbb);
    }else{
      nwxcP_xc_gss5tar(rb,gbb,gamma_css,css,&gcbb,
                       &Ggc_bb_rb,&Ggc_bb_gbb,
                       &Ggc_bb_rbrb,&Ggc_bb_rbgbb,&Ggc_bb_gbbgbb);
    }
  } else {
    gcbb = css[0] + css[1] + css[2]; // u -> 1 if s -> infinity
    gcbb = 0.0; // Elda(r)*gcbb -> 0 if r -> 0 hence OK to zero gcbb
    Ggc_bb_rb = 0.0; 
    Ggc_bb_gbb = 0.0; 
    Ggc_bb_rbrb = 0.0;
    Ggc_bb_rbgbb = 0.0; 
  ...
  if(ra > *tol_rho && rb > *tol_rho) {
    if(max_pow_u == 2 ){
      nwxcP_xc_gab3tar(ra,rb,gaa,gbb,gamma_cab, cab,
      ...
      nwxcP_xc_gab5tar(ra,rb,gaa,gbb,gamma_cab, cab,
                       &gcab,&Ggc_ab_ra,&Ggc_ab_rb,&Ggc_ab_gaa,&Ggc_ab_gbb,
                       &Ggc_ab_rara,&Ggc_ab_rarb,&Ggc_ab_rbrb,
                       &Ggc_ab_ragaa,&Ggc_ab_ragbb,&Ggc_ab_rbgaa,&Ggc_ab_rbgbb,
                       &Ggc_ab_gaagaa,&Ggc_ab_gaagbb,&Ggc_ab_gbbgbb);
    ...
    gcab = 0.0; // Elda(r)*gcab -> 0 if r -> 0 hence OK to zero gcab
    Ggc_ab_ra = 0.0;
    Ggc_ab_rb = 0.0;
    Ggc_ab_gaa = 0.0;
    Ggc_ab_gbb = 0.0;
...
@}
*/
/* $Id: nwxcP_xc_b97.c 25011 2013-12-19 17:45:25Z d3y133 $ */
