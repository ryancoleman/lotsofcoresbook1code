...
void iso_3dfd_it(float *ptr_next_base,  float *ptr_prev_base,  float *ptr_vel_base,   float *coeff,
	...
	#pragma omp parallel OMP_N_THREADS
	{
		...
		SIMD_TYPE tempVec_1, tempVec_2, tempVec_3;
		...
		register SIMD_TYPE divVec = SET_ZERO_INTR;
		SIMD_TYPE yNextVec, yPrevVec, zNextVec, zPrevVec; 
		SIMD_TYPE ySumVec, zSumVec, tempSumVec;

		SIMD_TYPE beforeVec, afterVec, currentVec;
		SIMD_TYPE cVec, velVec, nextVec;                     
            	SIMD_TYPE minusVec, plusVec, upVec;   
...
//-------------------------------------Cache Blocking Loop implementation End--------------------------------------
					...
					for(TYPE_INTEGER iz=bz; iz<izEnd; iz++) {
						for(TYPE_INTEGER iy=by; iy<iyEnd; iy++) {
							...
							int offset = iz*dimn1n2 + iy*n1 + bx;
