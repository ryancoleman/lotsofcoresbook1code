...
! Local variables
 INTEGER :: i, j, k, xb, yb, zb, Rb, DISTRO
 INTEGER :: IO_ERR, MPI_ERR
 INTEGER, DIMENSION (1:15) :: intParam
...
! Read parameter data from file in master node
 IF (proc == master) THEN
   ...
   intParam(10) = zmin
   intParam(11) = zmax
   intParam(12) = mpi_xdim
   intParam(13) = mpi_ydim
   intParam(14) = mpi_zdim
...
! Read bubble positions
   ...
   END IF
   intParam(15) = nBubbles
 END IF

! Broadcast values of parameters from the master to all other processors
 CALL MPI_BCAST(intParam, 15, MPI_INTEGER, master, MPI_COMM_WORLD, MPI_ERR)
...
! Assign parameter data to local-named variables in slave nodes
 IF (proc /= master) THEN
   ...
   zmin      = intParam(10)
   zmax      = intParam(11)
   mpi_xdim  = intParam(12)
   mpi_ydim  = intParam(13)
   mpi_zdim  = intParam(14)
   nBubbles  = intParam(15)
...
! Broadcast bubble(s) information from master to all other processors
 CALL MPI_BCAST(bubblesVec, nBubbles*4, MPI_DOUBLE_PRECISION, master, MPI_COMM_WORLD, MPI_ERR)
