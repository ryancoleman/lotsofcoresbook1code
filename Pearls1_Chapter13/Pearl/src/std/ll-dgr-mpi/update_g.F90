...
!! to the overlapping nodes in the coarser pressure/momentum grid for the
!! parallel dual grid D2Q9 Lee-Lin multiphase LBM.
...
!--------- Interpolate the density in the pressure/momentum mesh ---------------
 DO j = yl, yu
   DO i = xl, xu
! Source nodes for the interpolation
     xs = 2*i - 1
     ys = 2*j - 1
...
! Copy the values of the second derivative arrays and pre-multiply by kappaEf
...
! Rescale and copy values of the gradient of rho from the f grid to the g grid
