...
!> @details
!! Generates the final data file for the simulation in the dual grid parallel
!! D2Q5/D2Q9 Zheng-Shu-Chew multiphase LBM, which contains:
!!
!! - Input parameters
...
!!
!! All quantities are calculated in the order parameter grid and, when
!! necessary (sigma,IntWidth,Ref), scaled-down to the coarser momentum grid
...
!  Local Variables
 ...
 REAL(KIND = DBL) :: distroMem, auxMem, mpiMem, totalMem, memUnitF, memUnitG, Mb
...
! Pressure inside and outside the bubble, maximum velocity and effective radius
 DO j = yl_f, yu_f
   DO i = xl_f, xu_f
     ...
     IF ( phi_f(i,j) >= 0.D0 ) dataLocal(1) = dataLocal(1) + 1.D0
...
! Estimate memory usage (Mb)
 ...
 memUnitG = ( NX + 2 )*( NY + 2 )
 memUnitF = ( NX_f + 2 )*( NY_f + 2)

 memLocal(1) = 8.D0*( 18.D0*memUnitG + 10.D0*memUnitF )

 memLocal(2) = 8.D0*( 3.D0*memUnitG + 7.D0*memUnitF ) &
             + 4.D0*( 4.D0*( memUnitG + memUnitF ) )

 memLocal(3) = 8.D0*( 8.D0*( xsize_f + ysize_f ) + 4.D0*( xsize6 + ysize6 ) &
             + 40.D0 )
