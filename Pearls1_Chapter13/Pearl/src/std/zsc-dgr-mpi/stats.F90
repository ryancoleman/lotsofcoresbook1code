...
!! and write them to file "stats.out" for the dual grid parallel D2Q5/D2Q9
...
!  Common Variables
 USE NTypes,      ONLY : DBL
 USE Domain,      ONLY : invInitVol, invPi, iStep, tCall, xl_f, xu_f, yl_f, yu_f
...
!  Local Variables
 ...
 REAL(KIND = DBL) :: invVol, Pdif, Perr, Pin, Pout, R, Ro, Ref, Ux, Uy, Vef, Vol
 ...
 DO j = yl_f, yu_f
   DO i = xl_f, xu_f
     ...
     IF ( phi_f(i,j) >= 0.D0 ) THEN
       ...
       dataLocal(2) = dataLocal(2) + u_f(i,j,1)
       dataLocal(3) = dataLocal(3) + u_f(i,j,2)
     END IF
     ...
     ELSE IF ( R > (Ro + IntWidth) ) THEN
       dataLocal(6) = dataLocal(6) + p_f(i,j)
