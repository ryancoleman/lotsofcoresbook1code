...
!! pressure distribution function g in the parallel D2Q9 Lee-Lin multiphase LBM.
!!
...
! Common Variables
 USE NTypes,    ONLY : DBL
 USE Domain,    ONLY : inv12, ni, xl, xu, yl, yu
 USE FluidParams
 USE LBMParams
 IMPLICIT NONE
...
! Local Variables
 ...
 REAL(KIND = DBL) :: rhon, tau, invTau, AX, invCsRho
...
! Gradient of the square of the density gradient
     gradRhoSqX = 4.D0*( gradRhoSq(ie,j ) - gradRhoSq(iw,j ) ) &
                + gradRhoSq(ie,jn) - gradRhoSq(iw,js)          &
                + gradRhoSq(ie,js) - gradRhoSq(iw,jn)

     gradRhoSqY = 4.D0*( gradRhoSq(i ,jn) - gradRhoSq(i ,js) ) &
                + gradRhoSq(ie,jn) - gradRhoSq(iw,js)          &
                + gradRhoSq(iw,jn) - gradRhoSq(ie,js)
