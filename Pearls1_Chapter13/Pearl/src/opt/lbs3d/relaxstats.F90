...
! Calculate the velocity
       IF ( phi(m) >= 0.D0 ) THEN
       ux = ux + ( g1(mg)  - g2(mg)  + g7(mg)  - g8(mg)  + g9(mg) - g10(mg) &
          +        g11(mg) - g12(mg) + g13(mg) - g14(mg) )*invRhon

       uy = uy + ( g3(mg)  - g4(mg)  + g7(mg)  - g8(mg)  - g9(mg) + g10(mg) &
          +        g15(mg) - g16(mg) + g17(mg) - g18(mg) )*invRhon

       uz = uz + ( g5(mg)  - g6(mg)  + g11(mg) - g12(mg) - g13(mg) + g14(mg) &
          +        g15(mg) - g16(mg) - g17(mg) + g18(mg) )*invRhon
...
! Save velocity and effective radius and volume data
 ...
 IF ( IO_ERR == 0 ) THEN
   IF( iStep == 0 ) THEN
      WRITE(10,'(A)')"LBS3D v1.0"
      WRITE(10,'(A,7(1x,A))')"#Iter","Ux","Uy","Uz","Vef","Ref","Pdif","Perr"
   END IF
   WRITE(10,'(I9,7(1x,ES19.9))')iStep,ux,uy,uz,Vef,Ref,Pdif,Perr
   CLOSE(UNIT = 10)
 ELSE
   CALL MemAlloc(2)
