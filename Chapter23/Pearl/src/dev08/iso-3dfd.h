...
#define C_PREFETCH2(addr, PREF_DIST) _mm_prefetch((char const*) ((addr) + (PREF_DIST)), _MM_HINT_T1);
	...
	#define SET_ZERO_INTR _mm512_setzero_ps()

	#define SET_VALUE_INTR(v) _mm512_set1_ps(v)

	#define GENERATE_COEFFICIENT_ARRAY_INTR(i)                                      \
                C_PREFETCH(&coeff[i], 64, P1)                                           \
	...
	#define MUL_COEFF_INTR(vertical, front, coeff)                                  \
                ...
                C_PREFETCH(&ptr_prev[ix + vertical], P2, P1)                            \
                yNextVec = _mm512_load_ps(&ptr_prev[ix + vertical]);                    \
                                                                                        \
                __assume_aligned((void*)&ptr_prev[ix-vertical],     CACHELINE_BYTES);   \
                C_PREFETCH(&ptr_prev[ix - vertical], P2, P1)                            \
                yPrevVec = _mm512_load_ps(&ptr_prev[ix - vertical]);                    \
                ySumVec = _mm512_add_ps(yNextVec, yPrevVec);                            \
                                                                                        \
                __assume_aligned((void*)front,     CACHELINE_BYTES);                    \
                __assume_aligned((void*)&ptr_prev[ix+front],     CACHELINE_BYTES);      \
                C_PREFETCH(&ptr_prev[ix + front], P2, P1)                               \
                zNextVec = _mm512_load_ps(&ptr_prev[ix + front]);                       \
                                                                                        \
                __assume_aligned((void*)&ptr_prev[ix-front],     CACHELINE_BYTES);      \
                C_PREFETCH(&ptr_prev[ix - front], P2, P1)                               \
                zPrevVec = _mm512_load_ps(&ptr_prev[ix - front]);                       \
                                                                                        \
                zSumVec = _mm512_add_ps(zNextVec, zPrevVec);                            \
                                                                                        \
                tempSumVec = _mm512_add_ps(ySumVec, zSumVec);                           \
                                                                                        \
                divVec = _mm512_fmadd_ps(tempSumVec, coeff, divVec);
	...
	#define SHIFT_MULT_INIT								\
		...
		C_PREFETCH(&ptr_prev[ix-SIMD_STEP], 96, 48)				\
											\
	...
	#define SHIFT_MULT_INTR(ind)										\
		cVec = coeffVec[ind];   										\
				          									\
               	minusVec = (__m512)_mm512_alignr_epi32((__m512i)currentVec, (__m512i)beforeVec, SIMD_STEP-ind); 	\
               	plusVec = (__m512)_mm512_alignr_epi32((__m512i)afterVec, (__m512i)currentVec, ind); 		\
       	    	upVec = _mm512_add_ps(minusVec, plusVec);                 				\
               	divVec = _mm512_fmadd_ps(upVec, cVec, divVec);                 					
	...
	#define REFRESH_NEXT_INTR									\
		...
		C_PREFETCH1(&ptr_vel[ix], P1)								\
		velVec = _mm512_load_ps(&ptr_vel[ix]);							\
		tempVec_3 = _mm512_mul_ps(divVec, velVec);/*div*vel -> temp_3*/				\
                        		 								\
		__assume_aligned((void*)&ptr_next[ix],     CACHELINE_BYTES);				\
		C_PREFETCH2(&ptr_next[ix], 64)								\
		C_PREFETCH1(&ptr_next[ix], P1)								\
		nextVec = _mm512_load_ps(&ptr_next[ix]); 						\
		tempVec_1 = _mm512_mul_ps(twoVec, currentVec);/*2*prev -> temp_1*/			\
		tempVec_2 = _mm512_sub_ps(tempVec_1, nextVec);/* 2*prev - next -> temp_2*/		\
		nextVec = _mm512_add_ps(tempVec_3, tempVec_2);/*2*prev - next + div * vel -> next*/ 	\
		_mm512_storenr_ps(&ptr_next[ix], nextVec);
