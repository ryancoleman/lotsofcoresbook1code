...
int main()
{
...

    {
        for (size_t iter = 0; iter < LOOPS; ++iter) {
            TSLOG_INFO("\n>>> Buffer Type: COI_BUFFER_NORMAL" << ": " << iter);
            for (UnitMap::const_iterator iter = unitMap.begin(); iter != unitMap.end(); ++iter) {
                ...
                for (size_t i = 0; i < bufferSize.size(); ++i) {
                    ...
                    tstart = dtime();
                    CHECK_COI_RESULT(COIBufferCreate(bytes, COI_BUFFER_NORMAL, 0, data, 1, &proc, &buffer));
                    ttime = dtime() - tstart;
                    ...
                    tstart = dtime();
                    CHECK_COI_RESULT(COIBufferMap(buffer, 0, 0, COI_MAP_WRITE_ENTIRE_BUFFER,
                                     0, NULL, NULL, &mapInstance, &bufferData));

                    ttime = dtime() - tstart;
                    TSLOG_INFO("buffer map: " << bufferSize[i] << " " << iter->first << ", " << bufferSize[i]*toMB/(ttime) << " MB/s, " << ttime*1000 << " ms");
                    memcpy(data, cookie, sizeof(cookie));
                    memcpy(data + bytes - sizeof(cookieEnd), cookieEnd, sizeof(cookieEnd));

                    tstart = dtime();
                    CHECK_COI_RESULT(COIBufferUnmap(mapInstance, 0, NULL, NULL));
                    ttime = dtime() - tstart;
                    TSLOG_INFO("buffer unmap: " << bufferSize[i] << " " << iter->first << ", " << bufferSize[i]*toMB/(ttime) << " MB/s, " << ttime*1000 << " ms");

                    CHECK_COI_RESULT(COIBufferDestroy(buffer));
                    free(data);
                }
            }
        }
