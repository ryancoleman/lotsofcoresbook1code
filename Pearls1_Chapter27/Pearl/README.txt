...
Warning: if you have a Xeon Phi with less than 60 cores, edit RDBHilbert.hpp line 215, and replace the constant number `120' with the appropriate value for you (two times your number of available cores). This code controls the thread-pinning strategy and tries to maximise for throughput and load balance when using less than the full number of available threads (scattered affinity). The scheme will try to pin to non-existing cores if line 215 contains the wrong constant and more than 120 threads are requested via hardware.info, in which case the program will hang.
