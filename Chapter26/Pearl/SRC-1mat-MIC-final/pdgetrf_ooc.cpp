...
extern "C" 
#endif
void pdgetrf_ooc( int *m_in, int *n_in, 
...
if ((is_root)  && (idebug >= 0)) {
  printf("pdgetrf_ooc: mm %d nn %d mb %d nb %d isize %d memsize %d npanels %d nby %d\n",
             ...
             */
             ...
             if (has_work) {
               if (iinfo < 0) {
                 *info = iinfo;
                 if (idebug >= 1) {
                   printf("pdgetrf_ooc: pdgetrf return iinfo=%d\n", iinfo);
                 };
                 return;
                 };
               if (iinfo > 0) {
...
extern "C"
#endif
void pdgetrf_ooc_( int *m_in, int *n_in,
                 double *A, int *ia_in, int *ja_in, int *descA, 
                          int *ipiv_,  int *memsize_in, int *info )
{
  pdgetrf_ooc( m_in, n_in,
               A, ia_in, ja_in, descA,
