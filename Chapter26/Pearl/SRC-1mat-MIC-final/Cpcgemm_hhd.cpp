...
#define dC(i,j)  (((cuComplex *)C) + IDX2F((i),(j),descC[LLD_]))
#define dA(i,j)  (((cuComplex *)dAtmp) + IDX2F((i),(j),descAtmp[LLD_]))
#define dB(i,j)  (((cuComplex *)dBtmp) + IDX2F((i),(j),descBtmp[LLD_]))

static int nbytesAtmp = 0;
static float *Atmp = 0;
static cuComplex *dAtmp = 0;

static int nbytesBtmp = 0;
static float *Btmp = 0;
static cuComplex *dBtmp = 0;
...
/*
 * simulate PCGEMM but A, B are distributed matrices on host
 ...
 PROFSTART("Cpcgemm_hhd");
  ...
  if (is_k_small) {
    ...
    if (use_broadcast) {
      ...
      scope = 'R';
      if (mypcol == jc_proc) {
        scalapack_cgebs2d(&icontxt, &scope, &top, 
            &Locp, &Locq, Atmp, &lld );
        }
      else {
        scalapack_cgebr2d(&icontxt,&scope,&top,
            &Locp,&Locq, Atmp, &lld,
               &myprow, &jc_proc );
        };
    ...
    PROFEND("gemm:copy");
     ...
     zalpha = make_cuComplex( alpha[REAL_PART], alpha[IMAG_PART]);
     zbeta  = make_cuComplex( beta[REAL_PART], beta[IMAG_PART]);

     if (idebug >= 2) {
       printf("Cpcgemm_hhd: mm %d nn %d kk %d \n", mm,nn,kk);
 ...
 */
     if (idebug >= 1) {
          cuComplex *pAtmp = (cuComplex *) Atmp;
          cuComplex *pBtmp = (cuComplex *) Btmp;
          cuComplex aij,bij;
          int lld;
       ...
       CUBLAS_CGEMM(
          ...
          zalpha,  (cuComplex *) dA(lrA1,lcA1), ldAtmp,
                   (cuComplex *) dB(lrB1,lcB1), ldBtmp,
          zbeta,   (cuComplex *) dC(lrC1,lcC1), ldC );
    ...
    Cpcgemm_hhd( transA, transB, m,n,k1,
        alpha,   A, ia,ja,descA,
                 B, ib,jb,descB,
        beta,    C, ic,jc,descC );
    ...
    new_beta = one;
    Cpcgemm_hhd( transA, transB, m,n,k2,
...
extern "C"
#endif
void pcgemm_hhd( char *transA, char *transB,
          int *m, int *n, int *k,
          float *alpha, float *A, int *ia, int *ja, int *descA,
                         float *B, int *ib, int *jb, int *descB,
          float *beta,  cuComplex *C, int *ic, int *jc, int *descC )
