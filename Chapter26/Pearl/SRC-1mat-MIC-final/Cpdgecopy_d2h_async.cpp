...
extern "C"
#endif
void Cpdgecopy_general_async(int m, int n, 
...
zero[IMAG_PART] = 0.0;
 ...
 ldB = descB[LLD_];
  ...
  if (!is_ok) {
    printf("Cpdgecopy (%d,%d) :ia %d m %d descA[M_] %d  \n",
            myprow,mypcol,     ia,   m,   descA[M_] );
    printf("Cpdgecopy (%d,%d) :ja %d n %d descA[N_] %d \n",
            myprow,mypcol,     ja,   n,   descA[N_] );
    printf("Cpdgecopy (%d,%d) :ib %d jb %d descB[M_] %d descB[N_] %d\n",
        ...
        double *B, int ib, int jb, int *descB  )
{
   ...
   */
   ...
   Cpdgecopy_general_async(m,n,A,ia,ja,descA,B,ib,jb,descB, is_device_to_host );
} 
