...
extern "C" 
#endif
void pcgetrf_gpu(int *m_in, int *n_in, 
...
for( jstart=1; jstart <= minmn; jstart = jend + 1) {
  ...
  */
  ...
  PROFSTART("gpu:hA <- dA");
  Cpcgecopy_d2h( mm,nn, A,iia,jja,descA,  hA, ii,jj, desc_hA );
  PROFEND("gpu:hA <- dA");
  ...
  jj = 1;
  PROFSTART("gpu:A <- hA");
  Cpcgecopy_h2d(mm,nn, hA, ii,jj, desc_hA,
                       A, iia,jja, descA );
  PROFEND("gpu:A <- hA");
    ...
    if (nn >= 1) {
         ...
         for(kk=k1; kk <= k2; kk++) {
           ...
           if (kk != ip) {
               ...
               j1 = jja;
               j2 = jja;
               Cpcswap_gpu(nn, A,i1,j1,descA,inc1,
                               A,i2,j2,descA,inc2 );
                };
         };
      };
   ...
   */
   ...
   if ( (1 <= mm) && (1 <= nn)) {
     ...
     if (use_broadcast_triangular_matrix) {
       ...
       ii = 1;
       jj = 1;
       scalapack_pcgeadd( notrans, &mm, &mm, 
       ...
       CHKERR(cu_status );
        ...
        assert( Atmp != 0 );
         ...
         Cpcgecopy_d2h( mm,nn,A,iia,jja,descA,
          ...
          */
          ...
          Cpcgecopy_h2d( mm,nn, Atmp,ii,jj,desc_Atmp,
                             A, iia,jja, descA );
          PROFEND("gpu:A <- Atmp");
     };
	...
	kk = jb;
	 ...
	 cublasCgemm('N','N',mm,nn,kk,
        ...
        if (use_broadcast_triangular_matrix) {
          ...
          Cpcgecopy_d2h( kk,nn, A,iia,jja,descA, 
                                Atmp,1,1,desc_Atmp );
          PROFEND("gpu:Atmp <- A");
        };
...
extern "C"
#endif
void pcgetrf_gpu_( int *m_in, int *n_in,
           float *A, int *ia_in, int *ja_in, int *descA,
           int *ipiv_,  int *info )
{
    pcgetrf_gpu( m_in, n_in,
