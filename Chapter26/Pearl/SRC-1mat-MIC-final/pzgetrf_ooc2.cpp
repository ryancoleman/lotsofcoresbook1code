...
extern "C" 
#endif
void pzgetrf_ooc2( int *m_in, int *n_in, 
...
const int use_pzgetrf_gpu2 = TRUE;
...
/*
 * perform LU factorization similar to scalapack PZGETRF
...
if ((is_root)  && (idebug >= 0)) {
  printf("pzgetrf_ooc2: mm %d nn %d mb %d nb %d isize %d \n",
                        mm,nn, mb,nb, isize );
  printf("pzgetrf_ooc2: memsize %d npanels %d nby %d\n",
      memsize, npanels, nby );
};
...
#ifdef USE_CUBLASV2
{
 ...
 for(jstart=1; jstart <= n; jstart = jend + 1 ) {
     ...
     */
     ...
     PROFSTART("Y <- A");
     Cpzgecopy_h2d( mm,nn, A,iia,jja,descA,   dY, iy,jy,descY );
     PROFEND("Y <- A");
 ...
 */
              ...
              Cpzgecopy_d2h( mm,nn, dY, iy,jy,   descY, 
                                    hY, ihY,jhY, deschY );
              cublas_sync_stream();

              /*
                 Ztrsm('L','L','N','U', mm,nn,alpha,
              ...
              if (idebug >= 1) {
                ...
                Cpzlaprnt( mm,nn,hY,ihY,jhY,descY,hY_name);

                /*
                   { 
                   double * work;
                   size_t nbytes;
                   nbytes  = sizeof(double)*2*descY[MB_]*2;
                   nbytes = MAX(nbytes, 1024*1024);

                   work = (double *) malloc(  nbytes  );
                   ilen = strlen(hY_name);
                   scalapack_pzlaprnt( &mm,&nn,hY,&ihY,&jhY,deschY,
                    &irprnt,&icprnt,
                ...
                Cpzlaprnt(mm,mm,dX,iix,jjx,descX,dX_name);

                /*
                   {
                   double *work;
                   size_t nbytes;
                   nbytes =  sizeof(double)*2*descX[MB_]*2;
                   nbytes = MAX(nbytes, 1024*1024 );

                   work = (double *) malloc( nbytes );
                   ilen = strlen(dX_name);
                   scalapack_pzlaprnt( &mm,&mm, dX,&iix,&jjx,descX,
                     &irprnt,&icprnt,
               ...
               alpha = one;
               scalapack_pztrsm(side,uplo,trans,diag,
                           &mm,&nn, alpha,
                           dX,&iix,&jjx,descX,
                           hY,&ihY,&jhY,deschY );
 ...
 */
             ...
             if (has_work)  {
              ...
              if (idebug >= 1) {
                ...
                Cpzlaprnt(mm,kk,dX,iix,jjx,descX,cmatnm);
                ...
                /*
                   ...
                   nbytes = MAX( nbytes, 1024*1024 );
                   work = (double *) malloc( nbytes );
                  scalapack_pzlaprnt( &mm,&kk, dX, &iix,&jjx,descX, 
              ...
              if (use_immediate_copy_to_host) {
                ...
                if (use_new_hY) {
                 ...
                 scalapack_pzgeadd( notrans, &mm,&nn,
               ...
               */
                ...
                mm = ksize;
                nn = jsize;
                Cpzgecopy_h2d( mm,nn, hY,ihY,jhY,deschY,
                             dY, iy,jy,descY );
                 };
 ...
 */
            ...
            nn = jsize;
             ...
             /*
              * similar to pzgetrf( &mm,&nn,Y,j,1,&descY, &(ipivY(1)), &iinfo);
             */
             iy = j; 
             jy = 1;
             has_work = (mm >= 1) && (nn >= 1);
             if (has_work) {
               ...
               if (use_pzgetrf_gpu2) {
                 ...
                 PROFSTART("Y:pzgetrf_gpu2");
                 pzgetrf_gpu2( &mm_lu, &nn_lu, 
                   dY,&iy_lu,&jy_lu,descY, 
                   A, &iah0, &jah0, descA,
                   &(ipiv(1)), &iinfo );
                 PROFEND("Y:pzgetrf_gpu2");
                 ...
                 PROFSTART("Y:pzgetrf_gpu");
                 pzgetrf_gpu( &mm_lu, &nn_lu, 
                   dY,&iy_lu,&jy_lu,descY, &(ipivY(1)), &iinfo );
                 PROFEND("Y:pzgetrf_gpu");
               };
             };
             ...
             if (idebug >= 2) {
               char cmatnm[] = "dY2";
               if (is_root) { printf("after pzgetrf_gpu\n"); };
               Cpzlaprnt( mm,nn, (double *) dY,iy,jy,descY,cmatnm);
             };
#endif
             ...
             if (has_work) {
               if (iinfo < 0) {
                 *info = iinfo;
                 if (idebug >= 1) {
                   printf("pzgetrf_ooc2: pzgetrf return iinfo=%d\n", iinfo);
                 };
                 return;
                 };
               if (iinfo > 0) {
 ...
 */
            ...
            if (use_immediate_copy_to_host) {
              ...
              iia = (ia-1) + 1;
              jja = (ja-1) + jstart;
              Cpzgecopy_d2h( mm,nn,  dY,1,1,descY, 
                                   A,iia,jja,descA );
            };
 ...
 */
            ...
            for(int kk=k1; kk <= k2; kk++) {
              ...
              if (ip != kk) {
                 ...
                 j2 = j1;
                 if (nn >= 1) {
                   scalapack_pzswap( &nn,  A,&i1,&j1,descA,&inc1,
                                           A,&i2,&j2,descA,&inc2);
                   };
...
extern "C"
#endif
void pzgetrf_ooc2_( int *m_in, int *n_in,
                 double *A, int *ia_in, int *ja_in, int *descA, 
                          int *ipiv_,  int *memsize_in, int *info )
{
  pzgetrf_ooc2( m_in, n_in,
               A, ia_in, ja_in, descA,
