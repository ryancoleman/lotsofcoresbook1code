...
void Cpslaprnt( int m, int n, float *A, int ia, int ja, int *descA, char *cmatnm )
{
  ...
  if (is_root) {
    printf("Cpslaprnt: m %d n %d ia %d ja %d cmatnm %s\n",
                       m,   n,   ia,   ja,   cmatnm );
  };
  ...
  for(i=1; i <= m; i++) {
    for(j=1; j <= n; j++) {
      ...
      scalapack_pselget( &scope, &top, zalpha, A, &iia, &jja, descA );
      if (is_root) {
