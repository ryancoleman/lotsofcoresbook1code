...
void Cpdlaprnt( int m, int n, double *A, int ia, int ja, int *descA, char *cmatnm )
{
  ...
  if (is_root) {
    printf("Cpdlaprnt: m %d n %d ia %d ja %d cmatnm %s\n",
                       m,   n,   ia,   ja,   cmatnm );
  };
  ...
  for(i=1; i <= m; i++) {
    for(j=1; j <= n; j++) {
      ...
      scalapack_pdelget( &scope, &top, zalpha, A, &iia, &jja, descA );
      if (is_root) {
