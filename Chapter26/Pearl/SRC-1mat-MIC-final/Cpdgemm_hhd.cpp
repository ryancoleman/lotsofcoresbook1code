...
//PROFSTART("Cpdgemm_hhd");
  ...
  if (is_k_small) {
    ...
    if (use_broadcast) {
     ...
     PROFSTART("gemm:dist_copy_A");
     scalapack_pdgeadd( &transA, &m, &k, 
             new_alpha, A,&ia,&ja,descA,
             new_beta, Atmp,&iia,&jja,descAtmp );
     PROFEND("gemm:dist_copy_A");
     ...
     new_alpha = one;
     new_beta = zero;
     PROFSTART("gemm:dist_copy_B");
     scalapack_pdgeadd( &transB, &k, &n,
            new_alpha,  B, &ib, &jb, descB,
            new_beta, Btmp, &iib, &jjb, descBtmp );
     PROFEND("gemm:dist_copy_B");
     /*
      * copy buffer to GPU
      */
     ...
     inc2 = 1;
#ifdef USE_MIC
     PROFSTART("gemm:dist_h2d_B");
     offload_dSetVector(isizeBtmp, Btmp, inc1, dBtmp, inc2);
     PROFEND("gemm:dist_h2d_B");
#else
  #ifdef USE_CUBLASV2
     {
       PROFSTART("gemm:dist_h2d_Async_B");
       cublasStatus_t ierr;
       ierr = cublasSetVectorAsync( isizeBtmp, elemSize,
               Btmp, inc1, dBtmp, inc2, cublas_get_stream() );
       assert( ierr == CUBLAS_STATUS_SUCCESS );
       PROFEND("gemm:dist_h2d_Async_B");
     }
  #else
     PROFSTART("gemm:dist_h2d_B");
     cu_status = cublasSetVector(isizeBtmp, elemSize,
    ...
    PROFEND("gemm:copy");
	 ...
	 /* CHANGES */
//     zalpha = make_double( alpha[REAL_PART], alpha[IMAG_PART]);
//     zbeta  = make_double( beta[REAL_PART], beta[IMAG_PART]);
     zalpha = (double) alpha[REAL_PART];
     zbeta = (double) beta[REAL_PART];

     if (idebug >= 2) {
       printf("Cpdgemm_hhd: mm %d nn %d kk %d \n", mm,nn,kk);
 ...
 */
     if (idebug >= 1) {
          double *pAtmp = (double *) Atmp;
          double *pBtmp = (double *) Btmp;
          double aij,bij;
          int lld;
    ...
    Cpdgemm_hhd( transA, transB, m,n,k1,
        alpha,   A, ia,ja,descA,
                 B, ib,jb,descB,
        beta,    C, ic,jc,descC );
    ...
    new_beta = one;
    Cpdgemm_hhd( transA, transB, m,n,k2,
...
extern "C"
#endif
void pdgemm_hhd( char *transA, char *transB,
          int *m, int *n, int *k,
          double *alpha, double *A, int *ia, int *ja, int *descA,
                         double *B, int *ib, int *jb, int *descB,
          double *beta,  double *C, int *ic, int *jc, int *descC )
