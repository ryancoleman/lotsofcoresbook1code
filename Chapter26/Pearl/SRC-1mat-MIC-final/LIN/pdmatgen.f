     ...
     $                     ICNUM, MYROW, MYCOL, NPROW, NPCOL ) BIND(C)
*
...
*          if AFORM = 'F' : A is returned is a symmetric matrix.
*                           A is diagonal dominated. The element values are hardcoded here.
...
*
* A diagonally dominant matrix will be generated. The matrix has 
* hard-coded values. diagonal=10 off-diagonal=0.1
*

      IF( LSAME( AFORM, 'F' ) ) THEN
         JK = 1
         DO 480 IC = NOFF+1, NEND
            IOFFC = ((IC-1)*NPCOL+MRCOL) * NB
            DO 470 I = 1, NB
               IF( JK .GT. ICNUM ) GO TO 490
*
               IK = 1
               DO 450 IR = MOFF+1, MEND
                  IOFFR = ((IR-1)*NPROW+MRROW) * MB
*
                  IF( IOFFR .GT. IOFFC ) THEN
                     DO 420 J = 1, MB
                        IF( IK .GT. IRNUM ) GO TO 460
                        A(IK,JK) = 0.1
                        IK = IK + 1
  420                CONTINUE
*
...
*
         IK = 1
         DO 550 IR = MOFF+1, MEND
            IOFFR = ((IR-1)*NPROW+MRROW) * MB
            DO 540 J = 1, MB
               IF( IK .GT. IRNUM ) GO TO 560
               JK = 1
               DO 520 IC = NOFF+1, NEND
                  IOFFC = ((IC-1)*NPCOL+MRCOL) * NB
                  IF( IOFFC .GT. IOFFR ) THEN
                     DO 510 I = 1, NB
                        IF( JK .GT. ICNUM ) GO TO 530
                        A(IK,JK) = 0.1
                        JK = JK + 1
  510                CONTINUE
                  ELSE
