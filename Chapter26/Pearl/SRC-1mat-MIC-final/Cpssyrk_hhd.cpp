...
#define dC(i,j)  (((float *)C) + IDX2F((i),(j),descC[LLD_]))
#define dA(i,j)  (((float *)dAtmp) + IDX2F((i),(j),descAtmp[LLD_]))
#define dB(i,j)  (((float *)dBtmp) + IDX2F((i),(j),descBtmp[LLD_]))

static int nbytesAtmp = 0;
static float *Atmp = 0;
static float *dAtmp = 0;

static int nbytesBtmp = 0;
static float *Btmp = 0;
static float *dBtmp = 0;
...
extern "C"
#endif
void Cpssyrk_helper( char uplo, int n, int k,
                     float *alpha,  float *dAtmp,  int ia, int ja, int *descAtmp,
                                     float *dBtmp,  int ib, int jb, int *descBtmp,
                     float *beta,   float *C,      int ic, int jc, int *descC,
                     int myprow, int mypcol, int nprow, int npcol )
{

float zalpha = (float) alpha[REAL_PART];
float zbeta = (float) beta[REAL_PART];
      ...
      for(jstart=jcfirst; jstart <= jfinal; jstart = jend + (npcol-1)*nb + 1) {
           ...
           if (is_diagonal) {
              ...
              CUBLAS_SSYRK( 
                    ...
                    CUBLAS_OP_N,
                    nn, kk,
                    zalpha,  (float *) dA(lrA1,lcA1), ldAtmp,
                    zbeta,   (float *) dC(lrC1,lcC1), ldC );
#else
              CUBLAS_SSYRK( uplo, 'N', nn, kk,
                    zalpha,  (float *) dA(lrA1,lcA1), ldAtmp,
                    zbeta,   (float *) dC(lrC1,lcC1), ldC );
#endif
        ...
        */
            ...
            zalpha = (float) alpha[REAL_PART];
            zbeta = (float) beta[REAL_PART];
                 ...
                 float *  	beta, float *  	C, int   	ic, int   	jc, int *  	descC )
{
     ...
     *
     * simulate PSSYRK but A is distributed matrices on host
    ...
    float new_beta_[REAL_PART+IMAG_PART+1];
    float *new_beta = &(new_beta_[0]);
    float new_alpha_[REAL_PART+IMAG_PART+1];
    float *new_alpha = &(new_alpha_[0]);
    ...
    if (is_k_small) {
        ...
        if (nbytesAtmp <  nbytesA) {
            ...
            if (use_MallocHost) {
                Atmp = (float *) MallocHost( nbytesA );
            }
            else {
                Atmp = (float *) malloc( nbytesA );
            };
            assert( Atmp != 0 );
        ...
        if (nbytesBtmp < nbytesB ) {
            ...
            if (use_MallocHost) {
                Btmp = (float *) MallocHost( nbytesB );
            }
            else {
                Btmp = (float *) malloc( nbytesB );
            };
            assert( Btmp != 0 );
        ...
        if (use_broadcast) {
            ...
            scope = 'R';
            if (mypcol == jc_proc) {
                scalapack_sgebs2d(&icontxt, &scope, &top,
                                  &Locp, &Locq, Atmp, &lld );
            }
            else {
                scalapack_sgebr2d(&icontxt,&scope,&top,
                                  &Locp,&Locq, Atmp, &lld,
                                  &myprow, &jc_proc );
            };
    ...
    cublasHandle cu_handle; // using the new version of cublas_Ssyrk
    cublasFillMode_t CUDAUPLO;
            ...
            for ( st_col = st ; st_col < en ; st_col = st_col + delta ){
                ...
                // Do the sgemm on GPU, according to upper or lower
                sgemm_row = st_row + mb ; len_row = ic + m - sgemm_row ;
                if (upper) {len_row = st_row - ic; sgemm_row = ic; }
                
                
                local_extent( len_row,nb, sgemm_row,st_col,descC,
                             &LocpC, &LocqC, &lrC1,&lcC1, &lrC2,&lcC2 );
                
                jja = 1 ;
                local_extent( len_row,k, sgemm_row,jja,descAtmp,
                             &LocpA, &LocqA,
                             &lrA1,&lcA1, &lrA2,&lcA2 );
                iib = 1 ;
                local_extent( k,nb, iib,st_col,descBtmp,
        ...
        Cpssyrk_hhd( uplo, transA, m, n, k1,
                    alpha, A, ia, ja, descA,
                    beta , C, ic, jc, descC );
        ...
        Cpssyrk_hhd( uplo, transA, m, n, k2,
...
extern "C"
#endif
void pssyrk_hhd( char * 	uplo, char  *	transA, int *m, int  * 	n,	int *  	k,
                float *  	alpha, float *  	A, int  * 	ia, int *  	ja, int *  	descA,
                float *  	beta, float *  	C, int  * 	ic, int *  	jc, int *  	descC  )
{
    
    Cpssyrk_hhd(  *uplo, *transA, *m, *n, *k,
                  alpha, A, *ia, *ja, descA,
                  beta,  C, *ic, *jc, descC );
}
...
extern "C"
#endif
void pssyrk_hhd_( char *uplo, char *transA, int *m, int *n, int *k,
                  float *alpha, float *A, int *ia, int *ja, int *descA,
                  float *beta,  float *C, int *ic, int *jc, int *descC )
{
  pssyrk_hhd( uplo, transA, m,n,k,
              alpha,  A, ia, ja, descA,
              beta,   C, ic, jc, descC );
}
