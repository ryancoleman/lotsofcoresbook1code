...
#endif
void
Cpzlaswp_gpu( char direc, char rowcol, int n, 
...
*/
      if (is_row && is_forward ) {
             ...
             for(i=k1; i <= k2; i++) {
               ip = ipiv( iia + i - k1 );
               if (ip != i) {
                  Cpzswap_gpu(n, A, ia,ja,descA, descA[M_], A, ip,
                               ja, descA, descA[M_] );
               };
             }; /* end for i */
         };
...
*/
         if (is_row && (!is_forward)) {
           ...
           for(i=k2; i >= k1; i-- ) {
             ip = ipiv( iia + i - k1 );
             if (ip != i) {
               Cpzswap_gpu( n,A,i,ja,descA, descA[M_],  A, ip,
                            ja, descA, descA[M_] );
             };
           }; /* end for i */
         };
...
*/
         if ( (!is_row) && is_forward) {
               ...
               for(j=k1; j <= k2; j++ ) {
                 jp = ipiv( jja+j-k1 );
                 if (jp != j) {
                   Cpzswap_gpu( n,A,ia,j,descA,1,  A,ia,jp,
                                descA, 1 );
                 };
               }; /* end for j */
           };
