...
#if (0)
#define Cpzgemm_hhd(transA,transB,m,n,k, \
    alpha, A,ia,ja,descA, B,ib,jb,descB,beta, C,ic,jc,descC)  \
  scalapack_pzgemm(&transA, &transB, &m,&n,&k,  \
      alpha, A, &ia,&ja,descA, B, &ib,&jb,descB, beta, C, &ic,&jc, descC )
#endif
...
void pzgetrf_gpu2(int *m_in, int *n_in, 
   cuDoubleComplex *A, int *ia_in, int *ja_in, int *descA, 
...
if ((idebug >= 1) && (is_root)) {
  printf("pzgetrf_gpu2: m %d n %d ia %d ja %d \n",
      m,n,   ia,ja );
};
...
for( jstart=1; jstart <= minmn; jstart = jend + 1) {
  ...
  PROFSTART("gpu:hA <- dA");
  Cpzgecopy_d2h_async( mm,nn, A,iia,jja,descA,  Ah, ii,jj, desc_Ah );
  // cublas_sync_stream();
  PROFEND("gpu:hA <- dA");
  ...
  PROFSTART("d2h:U part");
   ...
   Cpzgecopy_d2h_async( mm,nn,A,iia,jja,descA,
         hA, iah, jah, desc_Ah );
  PROFEND("d2h:U part");
  }
  ...
  PROFSTART("gpu:pzgetrf");
  scalapack_pzgetrf( &mm_lu, &nn_lu, 
   ...
   */
   ...
   PROFSTART("pzgetrf_gpu2:d2h for swap");
   {
     ...
     for (i=1; i <= jsize; i++) {
      ...
      if (is_outside_block) {
        ...
        jah = (jah0-1) + (j+jb);
        if (!has_copied[irow]) {
          Cpzgecopy_d2h( 1, nn, A, iia,jja,descA, 
               hA, iah, jah, desc_Ah );
   ...
   PROFEND("pzgetrf_gpu2:d2h for swap");
   ...
   // cublas_sync_stream();
   PROFSTART("pzgetrf_gpu2: swap on CPU");
   {
     int inc1 = desc_Ah[M_]; 
     int inc2 = desc_Ah[M_];
   ...
   PROFEND("pzgetrf_gpu2: swap on CPU");
   ...
   PROFSTART("pzgetrf_gpu2:h2d for swap");
   {
     int irowmax =  (iah0-1) + jend;
     for (i=1; i <= jsize; i++) {
      int ip = lpvt(i); 
      ...
      if (is_outside_block) {
       ...
       jah = (jah0-1) + (j+jb);
       if (!has_copied[irow]) {
         Cpzgecopy_h2d_async( 1, nn,  hA,iah,jah, desc_Ah, A, iia,jja,descA);
         has_copied[irow] = TRUE;
         };
       };
     };
   ...
   PROFEND("pzgetrf_gpu2:h2d for swap");
         ...
         */
          ...
          if (use_split_gemm) {
            ...
            if ((mm1 >= 1) && (nn1 >= 1) && (kk1 >= 1)) {
              Cpzgemm_hhd( transA,transB,mm1,nn1,kk1,
            ...
            if ((mm2 >= 1) && (nn2 >= 1) && (kk2 >= 1)) {
              Cpzgemm_hhd( transA,transB,mm2,nn2,kk2,
                alpha, hA,iah,jah,desc_Ah,
                       hA,ii2,jj2,desc_Ah,
          ...
          Cpzgemm_hhd( transA, transB, mm,nn,kk, 
           alpha, hA, iah,jah, desc_Ah, 
                  hA, ii,jj,   desc_Ah, 
           beta,  A, iic,jjc, descA );
          };

          PROFEND("zgetrf_gpu2:pzgemm");
           };
...
extern "C"
#endif
void pzgetrf_gpu2_( int *m_in, int *n_in,
           double *A, int *ia_in, int *ja_in, int *descA,
           double *Ah, int *iah_in, int *jah_in, int *desc_Ah,
           int *ipiv_,  int *info )
{
    pzgetrf_gpu2( m_in, n_in,
             (cuDoubleComplex *) A, ia_in, ja_in, descA,
